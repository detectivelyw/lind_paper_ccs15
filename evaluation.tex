...
\cappos{Does the metric eval come here?  I thought we did that in Section 3?}
\cappos{It may be better to organize / frame this with questions.  I know 
we want to look at security, but can we be more specific?  Why is looking at
normal apps interesting?  Is reachability (i.e. fuzzing) the only thing
that matters?}
...
with other systems not using our design. The comparison results will show if our system is better, \cappos{Really?}
...
\cappos{We need to be sure to be very concrete when saying things like
`security'.  It is important to be crystal clear what we mean.}

\cappos{Do we need to fuzz Repy?  Do we need to argue what happens when
Repy or NaCl have flaws?}
